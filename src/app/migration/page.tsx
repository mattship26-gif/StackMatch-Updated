      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <p className="text-sm text-slate-500 mb-3">What you're migrating away from</p>
            {fromTool ? (
              <div className="flex items-center justify-between bg-slate-50 rounded-lg p-3">
                <div>
                  <div className="font-semibold text-slate-900">{fromTool.name}</div>
                  <div className="text-xs text-slate-500">{fromTool.category}</div>
                </div>
                <button onClick={() => setFromId('')} className="text-slate-400 hover:text-red-500 text-lg">×</button>
              </div>
            ) : (
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <p className="text-sm text-slate-500 mb-3">What you're considering switching to</p>
            {toTool ? (
              <div className="flex items-center justify-between bg-slate-50 rounded-lg p-3">
                <div>
                  <div className="font-semibold text-slate-900">{toTool.name}</div>
                  <div className="text-xs text-slate-500">{toTool.category}</div>
                </div>
                <button onClick={() => setToId('')} className="text-slate-400 hover:text-red-500 text-lg">×</button>
              </div>
            ) : (
              <div className="relative">
                <input
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 mb-8">
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            ...
            <div>
              <label className="block text-sm text-slate-600 mb-1">Current Monthly Cost ($)</label>
              <input type="number" value={monthlyRate} onChange={e => setMonthlyRate(Number(e.target.value))}
                className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-sm text-slate-600 mb-1">Contract Months Remaining</label>
          ...
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-6">
                <h3 className="font-semibold text-slate-500 text-sm mb-3">CURRENT COST</h3>
                <div className="text-3xl font-bold text-slate-900 mb-1">{analysis.currentCost.label}</div>
                <div className="text-sm text-slate-600">{fromTool.name} · {teamSize} users</div>
              </div>
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-6">
                <h3 className="font-semibold text-slate-500 text-sm mb-3">NEW COST</h3>
                <div className="text-3xl font-bold text-slate-900 mb-1">{analysis.newCost.label}</div>
                <div className="text-sm text-slate-600">{toTool.name} · {teamSize} users</div>
              </div>
            </div>
